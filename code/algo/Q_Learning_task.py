...
import numpy as np
import gym
from envs.JSP_v2 import env as make, make_env
from algo.algotirhms.Q_Learning import Q_learning
...
def get_args(env):
    ...
    parser.add_argument('--env_name',default='JSP-v2',type=str,help="name of environment")
    parser.add_argument('--n_envs',default=5,type=int,help="numbers of environments")
    parser.add_argument('--n_sample', default=100,type=int,help="num of sample points")
    parser.add_argument('--n_episodes',default=5,type=int,help="max steps of training")
    ...
    parser.add_argument('--tensorboard_log', default=curr_path + "/outputs/" + parser.parse_args().env_name + '/' + parser.parse_args().algo_name + \
            '/' + curr_time + '/tensorlog/')     
...
model = Q_learning(env=env, epsilon=0.1, gamma=cfg.gamma, learning_rate=cfg.lr)
# model = PPO.load("model/PPO-JSP-v3_20_20", env=env)
# model.learn(total_timesteps=cfg.n_episodes * cfg.n_steps * 200, tb_log_name="PPO-v5-random-mod")
for ep in range(cfg.n_sample):
    model.learn(total_timesteps=cfg.n_episodes * cfg.n_steps)
    total_makespan = 0 # sum of makespan under different envs
    for i in range(cfg.n_envs):
        obs = envs[i].reset()
        for j in range(cfg.n_steps):
            single_action = model.predict(obs)
            action = np.zeros(5)
...
print(f"Min makespan got from Q-learning is: {min_makespan}")
# plt.show()
