...
import numpy as np
import gym
# from envs.JSP_v0 import env0, make_env
# from envs.JSP_v1 import env1, make_env
# from envs.JSP_v2 import env2, make_env
from envs.JSP_v6 import env, make_env
...
model = SAC('MlpPolicy', env, learning_rate=cfg.lr, buffer_size=100000, batch_size=50, device=cfg.device, learning_starts=1000)
# model = SAC.load("model/SAC/SAC-JSP-v5-BFN", env=env)
for ep in range(cfg.n_sample):
    ...
    if ep % 10 == 0:
        ...
        logger.info(f"current reward is {round(ep_reward)}, current makespan: {round(ep_makespan)}")
...
print(f"Min makespan got from SAC is: {min_makespan}")
# plt.show()
